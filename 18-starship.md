# 18. Starship: настраиваем приглашение командной строки
...
18.1. Установите Starship и подключите его к fish. Убедитесь, что prompt изменился.

   При переходе в `terminal-quest` должна быть видна git-ветка.

18.2. Добавьте в `starship.toml` показ времени выполнения для команд дольше 200 мс. Проверьте на `sleep 1`.

   После `sleep 1` должно появиться время выполнения рядом с prompt.

18.3. Измените символ приглашения на любой другой. Проверьте, что он меняет цвет при ошибке.
