...
Иногда не нужно смотреть всё дерево целиком, а хочется быстро вытащить только объекты с определённым именем. Флаг `-P` оставляет в выводе только то, что подходит под шаблон.
...
Обрати внимание: `tree` оставил не только сами совпавшие файлы, но и папки, через которые к ним можно дойти.

Набери `tree -f -P '*anna*'` и посмотри, что осталось в дереве.
...
- `tree -P 'шаблон'` оставить только подходящие имена.
