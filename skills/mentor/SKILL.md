...
Example shape:
- "Здесь сначала подумайте про `find`, потому что задача про файлы, а не про строки."
- "Вам нужен не весь файл, а только количество строк. Посмотрите на `wc -l`."
...
- "Сначала вытащите строки, потом уже сортируйте. Порядок действий здесь важен."
